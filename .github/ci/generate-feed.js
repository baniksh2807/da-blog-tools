...
async function generateSingleFeed(config) {
  ...
  if (pathFilteredPosts.length === 0) {
    console.warn(` No posts found matching path "${TARGET_DIRECTORY}", skipping feed generation`);
    return;
  }
  ...
  const validPosts = pathFilteredPosts.filter(post => {
    ...
    if (!publishDate) {
      console.warn(` Post "${post.title}" has no date, skipping`);
      return false;
    }
    ...
    if (!timestamp || timestamp <= 0 || timestamp > Date.now() / 1000 + 86400) {
      console.warn(` Post "${post.title}" has invalid date: ${publishDate}, skipping`);
      return false;
    }
    ...
    if (isNaN(post.validDate.getTime())) {
      console.warn(` Post "${post.title}" date conversion failed: ${publishDate}, skipping`);
      return false;
    }
  ...
  if (validPosts.length === 0) {
    console.warn(' No posts with valid dates found, skipping feed generation');
    return;
  }
...
function filterPostsByPath(posts, targetDirectory) {
  if (!posts || !Array.isArray(posts)) {
    console.warn(' Invalid posts array for path filtering');
    return [];
  }
  ...
  const filteredPosts = posts.filter(post => {
    if (!post.path) {
      console.warn(` Post "${post.title}" has no path, skipping`);
      return false;
    }
